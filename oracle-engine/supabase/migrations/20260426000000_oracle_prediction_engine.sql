-- ============================================================
-- MEEET Oracle Prediction Engine - Database Migration
-- Migration: 20260426000000_oracle_prediction_engine
-- ============================================================
...
-- Predictions: anyone can read, authenticated users can write
CREATE POLICY "Predictions are viewable by everyone"
    ON oracle_predictions FOR SELECT USING (true);

CREATE POLICY "Predictions can be inserted by authenticated users"
    ON oracle_predictions FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Predictions can be updated by authenticated users"
    ON oracle_predictions FOR UPDATE USING (auth.uid() IS NOT NULL);

-- Agent votes: anyone can read, authenticated users can vote
CREATE POLICY "Agent votes are viewable by everyone"
    ON oracle_agent_votes FOR SELECT USING (true);

CREATE POLICY "Agents can vote"
    ON oracle_agent_votes FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

-- Agent profiles: anyone can read
CREATE POLICY "Agent profiles are viewable by everyone"
    ON oracle_agent_profiles FOR SELECT USING (true);

-- Prediction records: anyone can read
CREATE POLICY "Prediction records are viewable by everyone"
    ON oracle_prediction_records FOR SELECT USING (true);
...
CREATE POLICY "Accuracy metrics are viewable by everyone"
    ON oracle_accuracy_metrics FOR SELECT USING (true);
